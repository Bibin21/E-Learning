...
$query="select plan from propay where fname='$username'";
    ...
    <div class="modal-dialog modal-sm" id="cform">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title text-center form-title" style="color:white;">Payment Gateway</h4>            <button type="button" class="close" data-dismiss="modal" ><a href='courses.php' style='font-size: 30px;color:black;float:right;'>&times;</a></button>
        </div>
        <div class="modal-body padtrbl">
...
<!--                  password-->
  ...
  <?php  
              ...
              if($plan==0)
              {
                $query1 = "select * from courses where pro=0";
              }
              else
              {
  ...
  <h1  ><?php echo "$name" ?></h1>
  <input type="text" name="title" style="display:none" value='<?php echo "$name" ?>'>
 ...
 <p><button type="submit" onClick='showMessage(this.id)' name="delete" id='<?php echo "$pid" ?>' >Buy This Course</button></p>
</div>
<?php
              }
?>
