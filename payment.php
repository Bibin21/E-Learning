...
{
     echo"<center>"; 
        echo "<div class='card text-white bg-danger mb-3' style='max-width: 18rem;'><div class='card-header'>Transaction Failed!</div><div class='card-body'><h5 class='card-title'>Some input fields are  empty!</h5><p class='card-text'>All inputs are requires to Buy the course! </p><button class='btn-secondary'><a href='javascript:history.back()'>Go Back!</a></button></div></div>";
        echo "</center>"; 
}
if(strlen($cred)!=6)
{
    echo"<center>"; 
        echo "<div class='card text-white bg-danger mb-3' style='max-width: 18rem;'><div class='card-header'>Transaction Failed!</div><div class='card-body'><h5 class='card-title'>Credit card number is invalid!</h5><p class='card-text'>All inputs must be valid to Buy the course! </p><button class='btn-secondary'><a href='javascript:history.back()'>Go Back!</a></button></div></div>";
        echo "</center>"; 
}
else
{
...
$result=mysqli_query($conn,"select * from exptable where fname='$fname' and password='$password'")
    ...
    else if($row['fname']==$fname and $row['password']==$password)//for success
    {
        ...
        if(($coursename=="Pro Monthly")||($coursename=="Pro Yearly"))
        {
            $sql = "insert into propay(fname,plan) values('$fname','$coursename')";
	...
	else if($row['fname']!=$fname )
	{
		...
		echo"<center>"; 
        echo "<div class='card text-white bg-danger mb-3' style='max-width: 18rem;'><div class='card-header'>Transaction failed!</div><div class='card-body'><h5 class='card-title'>Username is incorrect!</h5><p class='card-text'>Only account holders can buy the courses! </p><button class='btn-success'><a href='home.php'>Create Account</a></button>&nbsp;&nbsp;&nbsp;&nbsp;<button class='btn-secondary'><a href='javascript:history.back()'>Go Back!</a></button></div></div>";
        echo "</center>";
	}
